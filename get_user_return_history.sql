DELIMITER $$
create procedure get_user_return_history(
...
begin
	...
	if not exists (select `tag` from `global`) then
		...
		if not ((select `type` from `account` where `username` = `user`) = 'User') then
			select * from `return_history` where `username` = `username_t`;
        else
...
drop procedure get_user_return_history;
call get_user_return_history('rohamzn');